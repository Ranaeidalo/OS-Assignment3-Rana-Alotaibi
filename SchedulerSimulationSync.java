...
class SharedResources {
    ...
    // Lock to protect execution log (ArrayList is not thread-safe)
    public static final ReentrantLock logLock = new ReentrantLock();

    // Semaphore to allow only ONE process to use CPU at a time
    ...
    public static void logExecution(String message) {
        // Protect ArrayList from concurrent modification
        logLock.lock();
        try {
            executionLog.add(message); // safely add log entry
        } finally {
...
class Process implements Runnable {
    ...
    public void run() {
        ...
        try {
            // Acquire CPU access (only one thread allowed)
        ...
        try {
            ...
            // Release CPU so another process can run
    ...
    public void runToCompletion() {
        // Acquire CPU before running the process
        try {
            SharedResources.cpuSemaphore.acquire();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } finally {
            // Release CPU after finishing execution
            SharedResources.cpuSemaphore.release();
        }
        try {
